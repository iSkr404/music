  ...
  <div class="NewsongList" v-infinite-scroll='scrollLoad' infinite-scroll-delay='1000'>
    <el-tabs class="tabs" v-model="activeName" @tab-click="handleClick">
      <el-tab-pane :label="item.name" :name="item.value +''" v-for="item in area" :key="item.value">
        <newsong-table v-if='activeName == item.value' :musiclist='musiclist'></newsong-table>
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
...
// 请求路由
import { _getTopSongs, _getSongsDetail, songDetail } from '@/network/discover/discover'
export default {
  data () {
    return {
      activeName: '0',
